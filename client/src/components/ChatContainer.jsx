...
const ChatContainer = ({ selectedUser, setSelectedUser }) => {
  ...
  const currentUserId = "680f50e4f10f3cd28382ecf9"; // 👈 Define current user
  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-xl">
      {/* Header */}
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-black">
        ...
        {messagesDummyData.map((msg, index) => (
          ...
          >
            {msg.image ? (
              ...
              <p
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-orange-950 text-white ${
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          <input
            ...
            className="flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-black"
          />
        ...
        <img src={assets.send_button} className="w-8 cursor-pointer" alt="" />
      </div>
    </div>
  ) : (
